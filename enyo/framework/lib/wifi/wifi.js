	...
	updateRow: function(inSender, inIndex) {
		...
		if (record) {
			...
			if (undefined !== record.networkInfo.connectState) {
				...
				case "notAssociated":
					if (record.networkInfo.ssid === this.joinedNetwork.ssid) {
						this.joinedNetwork = null;
					}
					if (record.networkInfo.ssid === this.selectedNetwork.ssid) {
						this.selectedNetwork = null;
					}
					break;
			...
			else if (null !== this.selectedNetwork && record.networkInfo.ssid === this.selectedNetwork.ssid) {
				this.$.itemStatus.setContent(rb.$L("CONNECTING..."));
				this.$.itemStatus.show();
            }
...
enyo.kind({
	...
	components: [
		...
		{kind: "RowGroup", caption: rb.$L("NETWORK"), name: "ipGroup", components: [
			{kind: "HFlexBox", components: [
		...
		{kind: "Pane", name: "pane", flex: 1, components: [
			{kind: "RowGroup", defaultKind: "HFlexBox", caption: rb.$L("IP ADDRESS"), components: [
				{tapHighlight: false, components: [
					{content: rb.$L("ADDRESS"), flex: 1},
					{name: "ipAddress", content: ""}
				]},
				{tapHighlight: false, components: [
					{content: rb.$L("SUBNET"), flex: 1},
					{name: "subnetAddress", content: ""}
				]},
				{tapHighlight: false, components: [
					{content: rb.$L("GATEWAY"), flex: 1},
					{name: "gatewayAddress", content: ""}
				]},
				{tapHighlight: false, components: [
					{content: rb.$L("DNS SERVER"), flex: 1},
					{name: "dns1Address", content: ""}
				]},
				{tapHighlight: false, components: [
					{content: rb.$L("DNS SERVER"), flex: 1},
					{name: "dns2Address", content: ""}
				]}
			]},
			...
			{kind: "RowGroup", defaultKind: "HFlexBox", caption: rb.$L("IP ADDRESS"), components: [
				{components: [
					{content: rb.$L("ADDRESS"), flex: 1},
					{kind: "Input", name: "ipField", onchange: "ipFieldChanged", spellcheck: false}
				]},
				{components: [
					{content: rb.$L("SUBNET"), flex: 1},
					{kind: "Input", name: "subnetField", onchange: "subnetFieldChanged", spellcheck: false}
				]},
				{components: [
					{content: rb.$L("GATEWAY"), flex: 1},
					{kind: "Input", name: "gatewayField", onchange: "gatewayFieldChanged", spellcheck: false}
				]},
				{components: [
					{content: rb.$L("DNS SERVER"), flex: 1},
					{kind: "Input", name: "dns1Field", onchange: "dns1FieldChanged", spellcheck: false}
				]},
				{components: [
					{content: rb.$L("DNS SERVER"), flex: 1},
					{kind: "Input", name: "dns2Field", onchange: "dns2FieldChanged", spellcheck: false}
				]}
			]},
		]},
		...
		if (undefined === this.joinedProfile.ipInfo ||
			(undefined !== profileInfo.useStaticIp &&
				true === profileInfo.useStaticIp) ) {
					this.displayStaticIpInfo();
				}
				else {
					this.displayDhcpInfo();
				}

				this.$.ipGroup.setCaption(profileInfo.ssid);
	},

	displayDhcpInfo: function() {
		var ipInfo = this.joinedProfile.ipInfo;

		this.$.dhcpToggleButton.setState(true);
		this.useStaticIp = false;
		this.$.pane.selectViewByIndex(0);

		this.$.ipAddress.setContent(ipInfo.ip);
		this.newIpAddress = ipInfo.ip;

		this.$.subnetAddress.setContent(ipInfo.subnet);
		this.newSubnet = ipInfo.subnet;

		if (undefined !== ipInfo.gateway) {
			this.$.gatewayAddress.setContent(ipInfo.gateway);
			this.newGateway = ipInfo.gateway;
		}
		if (undefined !== ipInfo.dns1) {
			this.$.dns1Address.setContent(ipInfo.dns1);
			this.newDns1 = ipInfo.dns1;
		}
		if (undefined !== ipInfo.dns2) {
			this.$.dns2Address.setContent(ipInfo.dns2);
			this.newDns2 = ipInfo.dns2;
		}
	},

	displayStaticIpInfo: function() {
		var ipInfo = this.joinedProfile.ipInfo;

		this.$.dhcpToggleButton.setState(false);
	...
	ipFieldChanged: function(inSender, inEvent) {
		this.newIpAddress = this.$.ipField.getValue();
	},

	subnetFieldChanged: function(inSender, inEvent) {
		this.newSubnet = this.$.subnetField.getValue();
	},

	gatewayFieldChanged: function(inSender, inEvent) {
		this.newGateway = this.$.gatewayField.getValue();
	},

	dns1FieldChanged: function(inSender, inEvent) {
		this.newDns1 = this.$.dns1Field.getValue();
	},

	dns2FieldChanged: function(inSender, inEvent) {
		this.newDns2 = this.$.dns2Field.getValue();
	},
	...
	handleDoneButton: function()	{
		...
		if (undefined === profileInfo.useStaticIp &&
			false === this.useStaticIp) {
			// nothing to do
		}
		else if (this.useStaticIp !== profileInfo.useStaticIp) {
			if (this.useStaticIp && this.validateIpSettings() ) {
				var ipInfo = {
